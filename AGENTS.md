...
| `npm run check`            | `npm run lint && npm run format:check && npm run typecheck && npm run deadcode && npm run dupcheck && npm run validate:agents && npm test` | Run every quality gate plus the test suite.      |
...
- **`quality`** (`.github/workflows/quality.yml`) — runs `lint`, `format:check`,
  `typecheck`, `deadcode`, `dupcheck`, and `validate:agents` (which checks this
  file's documented scripts, links, and paths still resolve).
